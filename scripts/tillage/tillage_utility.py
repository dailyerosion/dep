...
def operation_maker(crop,tillage_class,last_crop='C'):
    ...
    Croptype_dict={'C':'rowcrop','B':'rowcrop','G':'grass', 'P': 'grass'}        
    croptype=Croptype_dict[crop]
    
    Spring_op_list=[['notill','notill'],['fieldcultivate','fieldcultivate'],['fieldcultivate','fieldcultivate_disc'],
                     ['fieldcultivate_discx2','fieldcultivate'],['fieldcultivate_disc','fieldcultivate_disc']]
    ...
    with open('operations/spring_tillage/spring_%s_%s.txt' % (croptype,spring), 'r') as f:
        spring_tillage=f.readlines()
    with open('operations/planting/planting_%s_%s.txt' % (croptype,intensity), 'r') as f:
        planting=f.readlines()
    with open('operations/harvest/harvest_%s.txt' % (croptype), 'r') as f:
        harvest=f.readlines()
    with open('operations/fall_tillage/fall_%s_%s.txt' % (croptype,fall), 'r') as f:
