...
python proctor_tile_edit.py -s 0 --date="$(date --date '16 hours ago' +'%Y-%m-%d')" || exit 1

cd ../RT || exit 1
python enqueue_wepp_jobs.py -s 0 && python env2database.py -s 0 --date "$(date --date '16 hours ago' +'%Y-%m-%d')" && python spam_x.py
