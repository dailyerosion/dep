...
def calc_slope(arr):
    ...
    dx = (arr[1] - arr[0]) / 10.0  # TODO: use actual distance
    dy = (arr[2] - arr[1]) / 10.0  # TODO: use actual distance
...
slp = np.where(np.logical_and(x9 >= -0.03, x9 <= 0.03, slp < -999), x9slp, slp)
rng = np.where(np.logical_and(x7 >= -0.03, x7 <= 0.03, rng < -999), 7, rng)
slp = np.where(np.logical_and(x7 >= -0.03, x7 <= 0.03, slp < -999), x7slp, slp)
rng = np.where(np.logical_and(x3 >= -0.03, x3 <= 0.03, rng < -999), 3, rng)
slp = np.where(np.logical_and(x3 >= -0.03, x3 <= 0.03, slp < -999), x3slp, slp)
