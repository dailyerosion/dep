...
def plot1(df):
    ...
    ax.plot(gdf.index.values * 5., gdf['avg_det'].values, lw=2, color='k',
            label='Avg', zorder=5)
    ax.plot(gdf.index.values * 5., gdf['avg_det'].values, lw=4, color='w',
            zorder=4)
    ax.grid(True)
...
def main():
    ...
    ax.set_xlabel("Flowpath Length Floor [m], (average computed for len >= floor)")
