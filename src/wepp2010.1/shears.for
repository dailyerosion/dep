      subroutine shears(q,sslope,shear)
c
c     + + + PURPOSE + + +
...
c     rill width is set to zero in SR CONTIN
c
c     Using Gilley's relationship
c
      if (rwflag(iplane).eq.1) then
        wdthck = 1.13 * q ** 0.303
        if (width(iplane).lt.wdthck) width(iplane) = wdthck
      end if
c
      if (width(iplane).gt.rspace(iplane)) width(iplane) =
     1    rspace(iplane)
c
...
c
      if (q.le.0.) then
        ...
        u = (q/chezch/sqrt(sslope)) ** (2./3.) / width(iplane)
        dpthch = 0.2 * q ** .36
   10   dz = dpthch
        dpthch = u * (width(iplane)+dz+dz) ** (1./3.)
...
c
      xsarea = dpthch * width(iplane)
      wp = width(iplane) + 2.0 * dpthch
c
