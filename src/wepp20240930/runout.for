...
cw Modified by L. Wang, 12/26/2009
cw      if (ipeak.eq.2) write (59,2700)
      if (ipeak.ge.2) write (59,2700)
cw End of modifying
c
      do 10 ielmt = 1, nhill
     ...
     1      halpha(ielmt)
cw Modified by L. Wang, 12/26/2009
cw        if (ipeak.eq.2) write (59,2800) ielmt, htcs(ielmt)
        if (ipeak.ge.2) write (59,2800) ielmt, htcs(ielmt)
cw End of modifying
   10 continue
c
...
cw Added by L. Wang, 12/26/2009
            if (ipeak.ge.3) write (59,1700) ielmt, ieltyp(ielmt),
     1          idelmt(ielmt), htpk(ielmt)
cw End adding
          end if
c
...
c
 ...
 1100 format (/9x,60('-'))
 1200 format (//18x,'Hillslope elements: 1 - ',i4/)
 ...
 1410 format (/11x,'Element',5x,'Element',5x,'Channel/Impoundment',3x,
     1    'Time to',/,11x,'Number',7x,'Type',13x,'Number',10x,
     1    'Peak (hr)',/,11x,6('-'),7x,4('-'),13x,6('-'),10x,10('-'))
 1500 format (//18x,'Element',5x,'Element',5x,'Channel/Impoundment',/,18
     1    x,'Number',7x,'Type',13x,'Number',/,18x,6('-'),7x,4('-'),13x,6
     1    ('-'))
 1600 format (7x,i4,8x,a11,9x,i2,12x,f8.4,5x,f6.2)
 1700 format (12x,i4,8x,a11,9x,i4,12x,f8.4,5x,f6.2)
 1800 format (19x,i4,8x,a11,9x,i4)
 1900 format (19x,i4,8x,a11,9x,i4,//,18x,
 ...
 2200 format (//21x,'Runoff Volumes (m^3)',17x,'Peak',7x,'Peak',/,1x,
     ...
     1    7('-'))
 2300 format (f11.2,1x,f11.2,2x,f11.2,4x,f11.2,4x,f11.2,3x,f11.2,2x,
     1    f9.5,1x,f9.5)
 ...
 2800 format (21x,i4,8x,f8.2,10x,f6.2)
 2900 format (//18x,'Channel and Impoundment elements: ',i4,' - ',i4/)
      end
