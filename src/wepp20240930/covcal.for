...
CAS Testing
     !!   write(63,200) sdate,year,nowres, iresd(nowres,iplane),
     !!1  cf(iresd(nowres,iplane)),newcf(nowres),
     !!1  rigrm(nowres,iplane)
CAS Testing end
CAS Modified by A. Srivastava to use new weighted cf values
c       ------ sum of ridge flat residue cover
        area = area + newcf(nowres) * rigrm(nowres,iplane)
c       ------ sum of rill flat residue cover
        area2 = area2 + newcf(nowres) *
...
CAS Woody understory cover
      rigcov(iplane) = rigcov(iplane) + usinrc(iplane)
CAS Woody
...
CAS Woody understory cover
      rilcov(iplane) = rilcov(iplane) + usinrc(iplane)
CAS Woody
...
c
     ...
     1    rilcov(iplane)
      ...
      !!write(63,100) sdate, year, rescov(iplane)
c
      return
CAS
!!100   format(1x,2(1x,i5),15(1x,f16.5))
!!200   format(1x,4(1x,i5),15(1x,f16.5))
CAS
      end
