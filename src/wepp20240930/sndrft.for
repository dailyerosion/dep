...
c Modified by L. Wang and D. Shen, 04/22/2011.
c      if (ivers.ne.3.and.(azm(1) .lt. 0.1))  azm(1) = 360.0
c      if (ivers.ne.3.and.(azm(1) .gt. -0.1)) azm(1) = 360.0
c      if (ivers.eq.3.and.(azm(iplane) .lt. 0.1))  azm(iplane) = 360.0
c      if (ivers.eq.3.and.(azm(iplane) .gt. -0.1)) azm(iplane) = 360.0
      if (ivers.ne.3.and.(azm(1) > -0.1 .and. azm(1) < 0.1))
     1         azm(1) = 360.0
      if (ivers.eq.3.and.(azm(iplane) > -0.1 .and. azm(iplane) < 0.1))
     1         azm(iplane) = 360.0
c End modification.
...
c -- If Agricultural land...
        ...
        if (lanuse(iplane) .eq. 1) then
c DJ Shen modified (11/9/2011)        
c          basden = diam(irtype) * (ppop**0.5) / 25.0
          basden = ppop * 3.14 * (diam(irtype) / 2)**2
c end modification          
          trapef = exp(-0.1 * height) - 0.1
c DJ Shen modified (10/27/2011)
c         stor   = trapef * height * basden * rmasst/rmasso + rough
          stor   = trapef * height * (1-basden) * rmasst/rmasso + rough
c end modification          
...
c -- Basal density may have to be user-inputted for other than agland.
          ...
          trapef = exp(-0.1 * height) - 0.1
c DJ Shen modified (11/9/2011)           
c          stor   = trapef * height * basden + rough
          stor   = trapef * height * (1-basden) + rough
c end modifcation          
...
c DJ Shen modified (4/22/2011)    
c        vthg = -0.575 / ((1.0 - sin(atan(avgslp(iplane)/100)))
c     1         * (log(densg(iplane))) - 6.91)
