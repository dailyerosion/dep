...
c
                ...
                if (manver .lt. 2016.3) then !Added this condition to prevent parameters used going to zero for NRCS WEPP version --A. Srivastava 5/1/2018
                    jdherb(i,iplane) = 0
                    jdslge(i,iplane) = 0
                    jdburn(i,iplane) = 0
                    jdcut(i,iplane) = 0
...
c
                do 5, kk = 1, ncut(i,iplane)
                  cutday(kk,i,iplane) = cutda1(manndx,kk)
                  cutht(kk,i,iplane) = cutht1(manndx,kk)
   5           continue
c
c               estimate potential yield.
c               original code:
...
c               yld(i,iplane) = yild(1,i,iplane)
                jdancut(i,iplane) = cutday(1,i,iplane)
                ancutht(i,iplane) = cutht(1,i,iplane)
Cnew cutting ends
c
c
c           *** L1 ELSE-IF ***
...
c
CASnew
              if (mgtopt(i,iplane).eq.1.or.mgtopt(i,iplane).eq.4) then
CASnew
c
                ncut(i,iplane) = ncu1(manndx)
c
...
c
                ...
                nnc(iplane) = 1
              else if (mgtopt(i,iplane).eq.5) then
c               read(12,*) (gday(k,i,iplane), gend(k,i,iplane),
c               1                        animal(k,i,iplane), bodywt(k,i,iplane),
c               2                        area(iplane), digest(itype(i,iplane)),
c               3                        k = 1,ncycle(i,iplane))
                ncycle(i,iplane) = ncycl1(manndx)
                do 21 kk = 1, ncycle(i,iplane)
                  gday(kk,i,iplane) = gda1(manndx,kk)
                  gend(kk,i,iplane) = gen1(manndx,kk)
                  perremo(kk,i,iplane) = perremo1(manndx,kk)
   21           continue
c
                jdharv(i,iplane) = gend(1,i,iplane)
                nnc(iplane) = 1
              else if (mgtopt(i,iplane).eq.6) then
                ncycle(i,iplane) = ncycl1(manndx)
                do 22 kk = 1, ncycle(i,iplane)
                  gday(kk,i,iplane) = gda1(manndx,kk)
!                  gend(kk,i,iplane) = gen1(manndx,kk)
                  perremo3(kk,i,iplane) = perremo2(manndx,kk)
   22           continue
c
                jdharv(i,iplane) = gend(1,i,iplane)
                nnc(iplane) = 1
                
              else if (mgtopt(i,iplane).eq.7) then
                ncycle(i,iplane) = ncycl1(manndx)
                do 23 kk = 1, ncycle(i,iplane)
                  gday(kk,i,iplane) = gda1(manndx,kk)
!                  gend(kk,i,iplane) = gen1(manndx,kk)
                  perremo5(kk,i,iplane) = perremo4(manndx,kk)
   23           continue
c
                jdharv(i,iplane) = gend(1,i,iplane)
                nnc(iplane) = 1
...
c
          ...
          if (ui_run.eq.1) then
            ui_ssh(2,iplane) = ui_ssh(2,iplane) + ui_ssh1(i,iplane) * xx
          endif
c
...
c
          ...
          if (ui_run.eq.1) then
            ui_ssh(1,iplane) = ui_ssh(2,iplane)
          endif
...
c         -------- add soil layers below plow layer, to data arrays.
          do 80 i = 3, nsl(iplane)
            ...
            if (ui_run.eq.1) then
              ui_ssh(i,iplane) = ui_ssh1(ii,iplane)
            endif
