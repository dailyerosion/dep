...
c
      ...
      include 'cstruct.inc'
      include 'cupdate.inc'
c Added by L. Wang for channel routing, 12/26/2009.
      include 'cchrt.inc'
      include 'cslpopt.inc'
c End adding.
c
...
cw Added by L. Wang, 6/2/2010.
      if(ipeak <= 2) then
cw End adding
       if (peakot(ielmt).eq.0.0) then 
          write (*,'("wshpek div 0")')
       endif
       if (peakot(ielmt).gt.99999999) then
          write(*,'("wshpek inf")')
       endif
          
       if (peakot(ielmt).gt.0.0) then
