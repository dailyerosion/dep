...
c
     ...
     1    tgs(mxelem), trunsi, ppm, xchn, conv, sdr, wdet, tothill
...
c
     ...
     1      tgs(ielmt) * conv / 1000.0, cdety(ielmt)*conv,
     1      tronvy(ielmt),sbrvty(ielmt)
c
...
c
     ...
     1        trainy/1000), tirry, truny(nelmt), 
     1        tothill/1000,(wdet-tothill)/1000, tgs(nelmt)*conv/1000.0,
...
cd     1    12('-'),5x,2(10('-'),4x),/)
 1200 format (///'Channels',9x,'Discharge',5x,'Sediment',6x,'Soil',10x,
     1        'Upland',8x,'Subsuface Flow'/,
     1        'and',14x,'Volume',8x,'Yield',9x,'Loss',10x,'Charge',8x,
     1        'Volume' /,'Impoundments',5x,'(m^3)',9x,
     1         '(tonne)',7x,'(kg)',10x,'(m^3)',9x,'(m^3)'/,12('-'),5x,
     1         2(10('-'),4x),3(10('-'),3x),/)
 1300 format ('Hill    ',i3,5(2x,f12.1))
 1400 format ('Channel  ',i4,1x,5(f12.1,2x))
 1500 format ('Impoundment',1x,i2,1x,2(f12.1,2x))
 ...
 2300 format (/'Delivery From Channel Outlet:',/,
     ...
     1    f10.0,' m^3',/,
     1    'Total hillslope soil loss in contributing area  = ',
     1    f13.1,' tonnes',/,
     1    'Total channel soil loss                         = ',f13.1,
     1    ' tonnes',/,
