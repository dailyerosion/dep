...
c
      integer i, j
      real tmpdrf(mxhill),tmpdrv(mxhill)
      character*8 event
c
c     + + + LOCAL DEFINITIONS + + +
c
...
cx    End adding
c     Read tile drainage amounts - added 3/10/2010 - jrf
        read (49,1200) (tmpdrf(j),j = 1,nhill)
        read (49,1200) (tmpdrv(j),j = 1,nhill)
...
c         Add tile drainage water into subsurface amount for channel.
c         It will all be added to channel surface flow. 3/10/2010 - jrf          
          sbrunf(i) = sbrunf(i) + tmpdrf(i)
          sbrunv(i) = sbrunv(i) + tmpdrv(i)
cx    Added by Arthur Xu, 06/19/2000
...
c
      do 70 i = 1, nhill
c       Add hillslope tile into subsurface amount, will all
c       show up in channel surface, added 3/10/2010 - jrf        
        sbrunf(i) = sbrunf(i) + tmpdrf(i)
        sbrunv(i) = sbrunv(i) + tmpdrv(i)
