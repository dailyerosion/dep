      subroutine frsoil(sscunf,nowcrp)
c
c     +++PURPOSE+++
c
...
c                Get the approiate frozen soil conductivity value to use
c                base on the land use - 3/30/2012 - jrf                
                 kf = getFreezeCond(nowcrp,iplane)
                 
                 if (varsm .le. 0.01) then
c                frost heave
                      slks(j,i,iplane) = kf*sscunf(i,iplane)
                 else
                      call saxfun(i,varsm,varwtp, varkus)
                      if ((varkus/sscunf(i,iplane)).lt.kf) then
                         slks(j,i,iplane) = kf*sscunf(i,iplane)
                      else
